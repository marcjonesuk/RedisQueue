...
namespace ConsoleApplication2
{
    class Program
    {
        ...
        static void StartCircularConsumer(ConnectionMultiplexer cm, string q)
        {
            ...
            for (int i = 1; i <= 1; i++)
            {
                var ring = new RingBufferConsumer(cm.GetDatabase(), server, q, 1048576, "consumer" + Guid.NewGuid().ToString(), 
                    new ConsumerOptions(AckMode.Process, 5000, 5000));
                ...
                ring.AsObservable().Retry().Subscribe((a) =>
                {
                    count++;
                    c2++;
